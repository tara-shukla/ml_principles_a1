...
def test_part1():
    ...
    for tester, desc in test_ranges:
        ...
        for i in range(1000):
            ...
            assert(math.isclose(numerical_diff(TestFxs.f1, n),result1, rel_tol=1e-03))
            assert(math.isclose(backprop_diff(TestFxs.f1, n),result1, rel_tol=1e-03))


            result2 = TestFxs.df2dx(n)
            assert(math.isclose(numerical_diff(TestFxs.f2, n),result2, rel_tol=1e-03))
            assert(math.isclose(backprop_diff(TestFxs.f2, n),result2, rel_tol=1e-03))
            ...
            assert(math.isclose(numerical_diff(TestFxs.f3, n),result3, rel_tol=1e-03))
            assert(math.isclose(backprop_diff(TestFxs.f3, n),result3, rel_tol=1e-03))
...
def test_part2():
    ...
    for _ in range(1000):
        ...
        num_grad = numerical_grad(TestFxs.h2, v)
        end_num = time.perf_counter()
        num_time = end_num - start_num

        start_back = time.perf_counter()
        back_grad = backprop_diff(TestFxs.h2, v)
        end_back = time.perf_counter()
        back_time = end_back-start_back
        time_diffs.append( num_time-back_time)

        np.testing.assert_allclose(num_grad,back_grad, rtol=1e-03)

        dim2 = 1500
        v = np.random.uniform(1,10, size = (dim2))

        start_num = time.perf_counter()
        num_grad = numerical_grad(TestFxs.h3, v)
        end_num = time.perf_counter()
        num_time = end_num - start_num

        start_back = time.perf_counter()
        back_grad = backprop_diff(TestFxs.h3, v)
        end_back = time.perf_counter()
        back_time = end_back-start_back
        time_diffs.append(num_time-back_time)
    ...
    print("passed vectors part 2")

    
    #final time test
    
    v = np.random.uniform(1,10, size = (1500))

    print("Time Tests:")

    print("On Average, time diff numerical - backwards grad: ")
    ...
    end_num = time.perf_counter()
    num_time = end_num - start_num
    print("numerical gradient", num_time)

    start_back = time.perf_counter()
    back_grad = backprop_diff(TestFxs.h3, v)
    end_back = time.perf_counter()
    back_time = end_back-start_back
    print("backprop gradient: ", back_time)
    print("difference: ",num_time-back_time)
